...
public class JSqlParser {
    ...
    private static String getSelect(String sql, String columnName, String type) throws JSQLParserException {
        Statement statement = CCJSqlParserUtil.parse(sql);
        Select select = (Select) statement;
        PlainSelect plainSelect = (PlainSelect) select.getSelectBody();
        ...
        switch (type) {
            case "1":
                ...
                SelectItem newSelectItem = new SelectExpressionItem(newColumn);
                selectItems.add(newSelectItem);
                break;
            case "2":
                if (!selectItemsString.contains("org_name")) {
                    Column newColumnOrg = new Column("org_name AS '机构名称'");
                    SelectItem newSelectItemOrg = new SelectExpressionItem(newColumnOrg);
                    selectItems.add(newSelectItemOrg);
                }
                if (!selectItemsString.contains("sup_org_name")) {
                    Column newColumnSupOrg = new Column("sup_org_name AS '上级机构名称'");
                    SelectItem newSelectItemSupOrg = new SelectExpressionItem(newColumnSupOrg);
                    selectItems.add(newSelectItemSupOrg);
                }
                Expression where = plainSelect.getWhere();
    ...
    private static void extractColumnNamesFromExpression(Expression expression, Set<String> columnNames) {
        if (expression instanceof Column) {
            ...
            extractColumnNamesFromExpression(betweenExpression.getLeftExpression(), columnNames);
            // extractColumnNamesFromExpression(betweenExpression.getBetweenExpressionStart(), columnNames);
            // extractColumnNamesFromExpression(betweenExpression.getBetweenExpressionEnd(), columnNames);
        } else if (expression instanceof InExpression) {
            InExpression inExpression = (InExpression) expression;
            extractColumnNamesFromExpression(inExpression.getLeftExpression(), columnNames);
        } else if (expression instanceof IsNullExpression) {
            extractColumnNamesFromExpression(((IsNullExpression) expression).getLeftExpression(), columnNames);
            ...
            // You might need to parse the subquery further if needed
            ...
            for (Expression whenExpr : caseExpression.getWhenClauses()) {
                extractColumnNamesFromExpression(whenExpr, columnNames);
            }
            extractColumnNamesFromExpression(caseExpression.getElseExpression(), columnNames);
        } else if (expression instanceof Parenthesis) {
            Parenthesis parenthesis = (Parenthesis) expression;
            extractColumnNamesFromExpression(parenthesis.getExpression(), columnNames);
        }
    }
}
